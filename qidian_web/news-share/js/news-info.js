$(function() {
	...
	var deviceid = base.get_url_param("deviceid");
	// 日志记录
	var log ;
	var req_data = true;
			...
			base.jsonp_conn(path,rqd,function(data){
				...
				for(var c in contents){
					if(contents[c].txt){
						...
						news_html.push("<img style='width:"+ size.w +"px' src="+ contents[c].img +" />")
					}else if(contents[c].vid){
						var frame = $(contents[c].vid);
						news_html.push("<video src='"+frame[0].src+"' controls='controls'></video>")
					}
				}
				// 加载评论 
	...
	// 获取带广告的推荐信息
	function get_recomend_ad(nid,page){
