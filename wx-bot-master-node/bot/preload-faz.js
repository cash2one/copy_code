...
checkpage();
function checkpage(){
	var nowhost = document.location.host;
	if (nowhost=="wx.qq.com") {
...
// 拖拽滚动条获取全部群组信息
function get_all_group(callback){
	var groups = [];
	var start_item = $("#J_NavChatScrollBody>div.ng-scope>div.ng-scope[ng-repeat]").first();
	var item_num = $("#J_NavChatScrollBody>div.ng-scope>div.ng-scope[ng-repeat]").length+3;
	var left_cache = $("#J_NavChatScrollBody .bottom-placeholder.ng-scope").height();
	var scroll_timer = setInterval(function(){
		var scroll_now = $("#J_NavChatScrollBody").scrollTop();
		var items = start_item.nextAll('[ng-repeat]');
		if(scroll_now==0){
			var group = {};
			group.wxuin = "wxuin";
			group.wxname = "myname";
			group.sign = start_item.find("div.chat_item").attr("data-username");
			group.name = start_item.find('div.chat_item div.info span.nickname_text').text();
			groups.push(group);
		}
		items.each(function(){
			var group = {};
			group.wxuin = "wxuin";
			group.wxname = "myname";
			group.sign = $(this).find("div.chat_item").attr("data-username");
			group.name = $(this).find('div.chat_item div.info span.nickname_text').text();
			groups.push(group);
		})
		// 更新第一个元素
		start_item = $("#J_NavChatScrollBody>div.ng-scope>div.ng-scope[ng-repeat]").last();
		console.log(groups);
		// 滑动滚动条
		$("#J_NavChatScrollBody").scrollTop(scroll_now+64*item_num);
		if(left_cache<0){
			// callback(groups);
			$("#J_NavChatScrollBody").scrollTop(0);
			clearInterval(scroll_timer);
		}
		left_cache = left_cache-64*item_num;
	},1000);
}

// 拖动滚动条查找目标元素
function get_target_group(callback){
	var start_item = $("#J_NavChatScrollBody>div.ng-scope>div.ng-scope[ng-repeat]").first();
	var left_cache = $("#J_NavChatScrollBody .bottom-placeholder.ng-scope").height();
	var item_num = $("#J_NavChatScrollBody>div.ng-scope>div.ng-scope[ng-repeat]").length+3;
	var scroll_timer = setInterval(function(){
